...
export const opText = (operation_type, operation) => {
  ...
  if (operation_type === 0) {
    ...
    return axios
      .get(`${EXPLORER_URL}/account_name`, {
        params: { account_id: operation_account },
      })
      .then((response_name) => {
        // get me the to name:
        return axios
...
export const getAccountName = async (account_id) => {
  var account_name = '';
  const response = await axios.get(
    EXPLORER_URL + '/account_name?account_id=' + account_id,
  );
...
export const parseAuth = async (auth, type) => {
  ...
  await auth.map(async (value) => {
    ...
    if (type === 'key') {
      ...
      const response = await axios.get(
        EXPLORER_URL + '/account_name?account_id=' + value[0],
      );
      authline = {
        account: value[0],
        threshold: value[1],
...
export const parseVotes = async (votes) => {
  const results = await votes.map(async (value) => {
    ...
    const response = await axios.get(
      EXPLORER_URL + '/account_name?account_id=' + account,
    );
    return {
      id: value.id,
      type: type,
